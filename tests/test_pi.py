...
def skilled(project):
    ...
    (leaf / "coderabbit").symlink_to(CATALOG / "skills" / "coderabbit")
    return project
...
def test_the_target_is_relative_so_the_project_can_move(skilled):
    ...
    moment it is moved or cloned. Every other link kura makes points into the
...
def seat_repo(tmp_path, monkeypatch):
    """A fabricated catalog holding two plugins: one shipping an agent, one not.

    KURA_CATALOG is the seam, exactly as the kit fixture uses it, so
    converge_agents reads this store rather than the suite's own fixture catalog.
    """
    claude = tmp_path / "catalog"
    ...
    monkeypatch.setenv("KURA_CATALOG", str(claude))
    return claude
...
# `kura list` shows both plugins installed and Claude Code loads both from
...
def test_add_of_a_plugin_links_its_agents(catalog, effective, home, project):
    ...
    assert entry.is_symlink()
    source = CATALOG / "plugins" / "backend" / "agents" / "backend-staff-engineer.md"
    assert entry.resolve() == source.resolve()
...
def test_doctor_reports_plugin_agents_pi_cannot_read(plugged):
    ...
    `kura list` still shows the plugin installed and Claude Code loads it, so
...
def test_doctor_reports_the_occupied_path_for_every_shape_converge_refuses(
    ...
    both used to fall through to the missing-links branch: the note named `kura
...
def test_restore_converges_pi_when_there_was_nothing_to_install(
    ...
    """
    from kura.commands import restore
...
def test_adopt_converges_pi_for_a_project_that_predates_it(
    ...
    """adopt's whole population is projects set up before kura, which is exactly
    the population with no .agents/ link, and it wrote only the manifest."""
    from kura.commands import adopt

    add.install_one(catalog, effective, "skill", "coderabbit", False, home, project)
    # Roll the project back to the pre-kura shape: links on disk, no manifest,
    # and nothing pi can read.
    pi.link_path(project).unlink()
    state_file = project / ".claude" / "kura.json"
