...
def test_an_agents_skills_link_is_reported_as_the_trigger(tmp_path):
    """`add` creates this, so the prompt is a consequence of running kura here.
...
def test_each_named_pi_entry_is_reported_as_its_own_trigger(tmp_path, entry):
    """Not ours, so it must not be described as something kura did.
    ...
    assert reason and entry in reason and "kura" not in reason
