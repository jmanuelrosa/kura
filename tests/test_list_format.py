"""The exact bytes `kura list` prints, row shape by row shape.

The template came from the `claude-skill` / `claude-agent` fish functions kura
...
from kura import errors, scope
from kura import colors
from kura.commands import listing

from kit_helpers import CATALOG, SHIM
...
def test_groups_are_sorted_and_deduped(catalog, effective):
    ...
    """
    messy = cat.Artifact(name="messy", type=cat.SKILL, groups=("c", "a", "b", "a"), source=CATALOG)
...
def test_the_grouped_view_indents_members_under_a_tag(listing_run):
    ...
    assert f"    {DIM}·{RESET} commit {DIM}(global){RESET}" in out
