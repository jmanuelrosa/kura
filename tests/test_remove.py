...
from kura import errors, scope, state
from kura.commands import add, remove
...
def test_d11_a_global_dependency_is_never_a_candidate(catalog, effective, home, project):
    ...
    kura in ~/work/api cannot see ~/work/web, so it cannot know the global
    ...
    removal = remove.plan(
        ...
        add.provenance_entries(plan_), no_cascade=False, claude=CATALOG,
    )
...
def test_d12_removing_a_global_artifact_cascades_nothing(catalog, effective, home, project):
    ...
    removal = remove.plan(
        catalog, SKILL, "grill-me", True, home, project, {}, no_cascade=False, claude=CATALOG,
    )
...
def test_d12_a_global_removal_cannot_reach_into_a_project(catalog, effective, home, project):
    ...
    removal = remove.plan(
        catalog, SKILL, "grill-me", True, home, project, provenance, no_cascade=False, claude=CATALOG,
    )
...
def test_d14_an_untracked_dependency_is_kept():
    ...
    assert doomed == []
    assert kept == [("dep", ["not tracked by kura"])]
...
def test_d1_the_symlink_goes_and_the_source_stays(catalog, effective, home, project):
    ...
    removal = remove.plan(catalog, SKILL, PROJECT_SKILL, False, home, project, {}, False, CATALOG)
    remove.apply(removal, project)

    assert not (project / ".claude" / "skills" / PROJECT_SKILL).exists()
    assert (CATALOG / "skills" / PROJECT_SKILL).is_dir(), "the repo source must be untouched"
...
def test_d2_a_global_skill_is_removed_from_home(catalog, effective, home, project):
    ...
    removal = remove.plan(catalog, SKILL, GLOBAL_SKILL, True, home, project, {}, False, CATALOG)
    remove.apply(removal, None)
...
def test_d3_an_unknown_name_is_not_found(catalog, effective, home, project):
    removal = remove.plan(catalog, SKILL, "no-such-thing", False, home, project, {}, False, CATALOG)
...
def test_d6_an_agent_is_removed_with_its_suffix(catalog, effective, home, project):
    ...
    removal = remove.plan(catalog, AGENT, "architect", True, home, project, {}, False, CATALOG)
    remove.apply(removal, None)
    assert not (home / ".claude" / "agents" / "architect.md").exists()


def test_d7_no_project_is_refused(catalog, effective, home):
    removal = remove.plan(catalog, SKILL, PROJECT_SKILL, False, home, None, {}, False, CATALOG)
...
def test_d7_matches_adds_refusal_so_the_two_agree(catalog, effective, home):
    ...
    removed = remove.plan(catalog, SKILL, PROJECT_SKILL, False, home, None, {}, False, CATALOG)
...
def test_d15_no_cascade_removes_only_the_named_artifact(catalog, effective, home, project):
    ...
    removal = remove.plan(
        ...
        no_cascade=True, claude=CATALOG,
    )
    assert removal.cascaded == []
    remove.apply(removal, project)
...
def test_d16_provenance_is_dropped_for_everything_removed(catalog, effective, home, project):
    ...
    removal = remove.plan(
        catalog, SKILL, MIXED_PARENT, False, home, project, provenance, False, CATALOG
    )
    remove.apply(removal, project)
...
def test_d17_the_state_file_goes_when_nothing_is_left(catalog, effective, home, project):
    ...
    removal = remove.plan(
        catalog, SKILL, PROJECT_SKILL, False, home, project, state.read(project), False, CATALOG
    )
    remove.apply(removal, project)
...
def test_a_group_expansion_ignores_a_real_directory(catalog, effective, home, project):
    """Only links kura made are candidates, as the named path already holds."""
...
def test_end_to_end_removing_from_a_directory_outside_any_repo(kit, tmp_path):
    ...
    assert not (plain / ".claude" / "kura.json").exists()
