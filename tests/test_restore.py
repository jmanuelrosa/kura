"""Group J: `restore`, installing what kura.json already records.
...
def step(name, kind=SKILL, required_by=None):
    from kura.commands.add import Step
...
def plan_with(*steps, refused=False):
    from kura.commands.add import Plan
...
def test_j4_no_manifest_points_at_adopt(kit, project):
    ...
    assert "kura adopt" in result.stderr
...
def test_j4_restore_in_home_refuses(kit):
    ...
    assert "kura sync" in result.stderr
