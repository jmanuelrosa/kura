...
from kura import checks, errors, scope, state
from kura.commands import add, doctor
from kit_helpers import CATALOG
...
REAL_SKILL_SOURCE = CATALOG / "skills" / "coderabbit"
...
def test_g3_the_real_repo_has_nothing_untracked(catalog):
    assert checks.untracked_on_disk(catalog, CATALOG) == []
...
def test_g1_a_broken_symlink_is_reported(tmp_path):
    ...
    findings = checks.broken_links(CATALOG, home, project)
    assert len(findings) == 1
...
def test_g1_a_broken_link_in_home_is_reported(tmp_path):
    ...
    skills.mkdir(parents=True)
    (skills / "gone").symlink_to(tmp_path / "nowhere")
    findings = checks.broken_links(CATALOG, home, None)
...
def test_g1_a_healthy_link_is_not_reported(tmp_path):
    ...
    assert checks.broken_links(CATALOG, home, project) == []
...
def test_g10_a_global_artifact_in_a_project_is_a_problem(catalog, effective, tmp_path):
    ...
    findings = by_check(checks.wrong_scope(catalog, effective, home, project, CATALOG), "wrong-scope")
    assert len(findings) == 1
    assert findings[0].is_problem
...
def test_g11_an_untagged_artifact_in_home_is_only_a_note(catalog, effective, tmp_path):
    ...
    findings = by_check(checks.wrong_scope(catalog, effective, home, project, CATALOG), "global-override")
    assert len(findings) == 1
...
def test_a_correctly_placed_pair_yields_nothing(catalog, effective, tmp_path):
    ...
    for art, where, root in ((global_art, scope.GLOBAL, home), (project_art, scope.PROJECT, project)):
        ...
        target.symlink_to(art.source)
    assert checks.wrong_scope(catalog, effective, home, project, CATALOG) == []
...
def test_g12_a_record_with_no_link_is_a_problem(catalog, tmp_path):
    ...
    findings = by_check(checks.provenance_drift(catalog, provenance, project, CATALOG), "stale-provenance")
    assert len(findings) == 1
    assert findings[0].is_problem
...
def test_g13_a_link_with_no_record_is_a_note(catalog, tmp_path):
    ...
    findings = by_check(checks.provenance_drift(catalog, {}, project, CATALOG), "untracked-install")
    assert len(findings) == 1
    assert not findings[0].is_problem
    assert "cascade will keep it" in findings[0].detail
    assert "kura adopt" in findings[0].detail, "the fix should be named"
...
def test_g14_a_dependency_nothing_needs_is_reported_as_removable(catalog, tmp_path):
    ...
    skills = project / ".claude" / "skills"
    skills.mkdir(parents=True)
    (skills / "test-driven-development").symlink_to(CATALOG / "skills" / "test-driven-development")
    provenance = {(SKILL, "test-driven-development"): state.dep_of("spec-driven-development")}

    findings = by_check(checks.provenance_drift(catalog, provenance, project, CATALOG), "removable")
    assert len(findings) == 1
    assert not findings[0].is_problem
    assert "kura remove test-driven-development --type skill" in findings[0].detail
...
def test_g14_a_dependency_still_needed_is_not_reported(catalog, tmp_path):
    ...
    for name in ("spec-driven-development", "test-driven-development"):
        (skills / name).symlink_to(CATALOG / "skills" / name)
    provenance = {
        (SKILL, "spec-driven-development"): state.DIRECT,
        (SKILL, "test-driven-development"): state.dep_of("spec-driven-development"),
    }
    assert by_check(checks.provenance_drift(catalog, provenance, project, CATALOG), "removable") == []
...
def test_provenance_drift_is_skipped_with_no_project(catalog):
    assert checks.provenance_drift(catalog, {(SKILL, "x"): state.DIRECT}, None, CATALOG) == []
    ...
    doctored = {
        **catalog,
        (PLUGIN, "broken"): cat.Artifact(
            name="broken", type=PLUGIN, manifest_keys=("name", "dependencies"), source=CATALOG
        ),
        (SKILL, "ghost"): cat.Artifact(name="ghost", type=SKILL, source=tmp_path / "nowhere"),
    }
    plugin_only = doctor.collect(doctored, effective, CATALOG, home, project, {}, config={}, kind=PLUGIN)
...
def test_g15_cross_type_findings_are_dropped_when_narrowing(catalog, effective, tmp_path):
    ...
    full = doctor.collect(doctored, effective, CATALOG, home, None, {}, config={})
    narrowed = doctor.collect(doctored, effective, CATALOG, home, None, {}, config={}, kind=SKILL)
...
def test_a6_an_unnarrowed_run_includes_all_three_types(catalog, effective, tmp_path):
    ...
    findings = doctor.collect(doctored, effective, CATALOG, home, None, {}, config={})
...
def test_g17_a_clean_run_exits_ok(catalog, effective, tmp_path, capsys):
    home = tmp_path / "home"
    findings = doctor.collect(catalog, effective, CATALOG, home, None, {}, config={})
...
def test_end_to_end_detects_a_wrong_scope_install(kit, project):
    ...
    skills = project / ".claude" / "skills"
    skills.mkdir(parents=True)
    (skills / "commit").symlink_to(CATALOG / "skills" / "commit")
