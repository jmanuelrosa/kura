...
def artifact_files():
    ...
    """
    agents = [p for p in CATALOG.rglob("*.md") if p.parent.name == "agents"]
    return sorted({*CATALOG.rglob("SKILL.md"), *agents})


def test_every_real_artifact_ships_a_block_the_scanner_accepts():
    """The fixture corpus, held to the one-directional contract.

    Small on purpose: the evidence that matters against a false problem is a real
    catalog's hundred-odd hand-written blocks, and that sweep belongs to whoever owns
    the catalog rather than to this repository. Here it guards the fixture, so a block
    added to it cannot quietly be one the scanner objects to.
    """
    offenders = []
    for path in artifact_files():
        block = frontmatter.block(path.read_text())
        if block is None:
            offenders.append(f"{path.relative_to(CATALOG)}: no frontmatter block")
            continue
        _, malformed = scanned(f"{block}\n")
        if malformed and not rejects(block):
            offenders.append(f"{path.relative_to(CATALOG)}: called malformed, but PyYAML accepts it")
        elif not malformed and rejects(block):
            offenders.append(f"{path.relative_to(CATALOG)}: PyYAML rejects this and the scanner missed it")
    assert offenders == [], "\n".join(offenders)
