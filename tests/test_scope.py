...
def test_installed_names_lists_only_symlinks(tmp_path):
    ...
    (skills / "linked").symlink_to(CATALOG / "skills" / "coderabbit")
    (skills / "handmade").mkdir()
    assert scope.installed_names(project, cat.SKILL, CATALOG) == {"linked": skills / "linked"}
...
def test_installed_names_strips_the_agent_suffix(tmp_path):
    ...
    (agents / "architect.md").symlink_to(CATALOG / "agents" / "architect.md")
    (agents / "notes.txt").symlink_to(CATALOG / "agents" / "architect.md")
    assert scope.installed_names(project, cat.AGENT, CATALOG) == {"architect": agents / "architect.md"}


def test_installed_names_handles_a_missing_directory(tmp_path):
    assert scope.installed_names(tmp_path / "nope", cat.SKILL, CATALOG) == {}
    assert scope.installed_names(None, cat.SKILL, CATALOG) == {}
...
def test_installed_names_ignores_a_link_pointing_outside_the_store(tmp_path):
    ...
    assert scope.installed_names(project, cat.SKILL, CATALOG) == {}
...
def test_a_plugin_link_is_not_counted_as_a_skill(tmp_path):
    ...
    skills = project / ".claude" / "skills"
    skills.mkdir(parents=True)
    (skills / "backend").symlink_to(CATALOG / "plugins" / "backend")

    assert scope.installed_names(project, cat.PLUGIN, CATALOG) == {"backend": skills / "backend"}
    assert scope.installed_names(project, cat.SKILL, CATALOG) == {}
...
def test_a_skill_link_is_not_counted_as_a_plugin(tmp_path):
    ...
    skills = project / ".claude" / "skills"
    skills.mkdir(parents=True)
    (skills / "coderabbit").symlink_to(CATALOG / "skills" / "coderabbit")

    assert scope.installed_names(project, cat.SKILL, CATALOG) == {"coderabbit": skills / "coderabbit"}
    assert scope.installed_names(project, cat.PLUGIN, CATALOG) == {}
...
def test_installed_scope_disambiguates_by_store_too(tmp_path):
    ...
    assert scope.installed_scope(plugin, home, project, CATALOG) == scope.PROJECT
    assert scope.installed_scope(same_name_skill, home, project, CATALOG) is None
...
def test_all_links_returns_every_symlink_including_foreign_ones(tmp_path):
    ...
    skills.mkdir(parents=True)
    agents.mkdir(parents=True)
    (skills / "ours").symlink_to(CATALOG / "skills" / "coderabbit")
    (skills / "foreign").symlink_to(tmp_path / "elsewhere")
    (agents / "an-agent.md").symlink_to(CATALOG / "agents" / "architect.md")
...
def test_points_into_classifies_a_broken_link_by_its_intended_target(tmp_path):
    ...
    link.symlink_to(CATALOG / "skills" / "deleted-upstream")
    assert not link.exists()
    assert scope.points_into(link, CATALOG / "skills") is True
