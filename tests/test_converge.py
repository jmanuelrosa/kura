"""Group C: `kura converge`, and the discovery that makes `--all` mean something.
...
from kura import checks, errors, pi, projects
from kura.cli import build_parser
from kura.commands import converge
from kit_helpers import CATALOG


SKILL = CATALOG / "skills" / "coderabbit"
...
def test_either_mark_is_enough(tmp_path):
    ...
    this command exists for: the ones provisioned before kura wrote a manifest."""
    manifest_only = tmp_path / "a"
    (manifest_only / ".claude").mkdir(parents=True)
    (manifest_only / ".claude" / "kura.json").write_text("{}")
...
def test_a_project_with_nothing_pi_could_see_is_dropped(home, tmp_path):
    ...
    (commands / ".claude" / "kura.json").write_text("{}")
...
def test_the_pi_notes_name_the_command_that_only_converges(home, tmp_path):
    ...
    assert "kura converge" in detail
    assert "kura add" not in detail
