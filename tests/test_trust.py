...
Two of these are not kura's rules to choose, and that is what most of this file
...
from kura import checks, errors, workspace as ws
from kura.commands import trust
...
def test_clearing_under_a_trusted_ancestor_says_so_and_still_exits_ok(tmp_path):
    ...
    assert f"kura trust --off {tmp_path}" in text
...
def test_g18_a_linked_plugin_in_an_untrusted_workspace_is_a_problem(tmp_path):
    project = plugin_project(tmp_path, CATALOG)
    findings = checks.untrusted_workspace({}, project, CATALOG)
...
def test_g18_inherited_trust_satisfies_the_check(tmp_path):
    ...
    assert checks.untrusted_workspace(config, project, CATALOG) == []


def test_g18_a_project_with_no_plugins_is_never_asked_about_trust(tmp_path, project):
    """Trust is Claude Code's business until something kura installed depends on
    it. Reporting it everywhere is how a report earns being skipped."""
    assert checks.untrusted_workspace({}, project, CATALOG) == []


def test_g18_is_skipped_outside_a_project():
    assert checks.untrusted_workspace({}, None, CATALOG) == []
