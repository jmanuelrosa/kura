...
def test_c1_a_project_skill_lands_in_the_project(catalog, effective, home, project):
    ...
    assert link.resolve() == (CATALOG / "skills" / PROJECT_SKILL).resolve()
...
def test_c5_the_refusal_prints_the_corrected_command(catalog, effective, home, project):
    plan_ = make_plan(catalog, effective, cat.SKILL, GLOBAL_SKILL, home=home, project=project)
    assert f"kura add {GLOBAL_SKILL} --type skill --global" in plan_.message
...
def test_c7_no_pin_record_is_written_anywhere(catalog, effective, home, project):
    ...
    assert not (home / ".claude" / "kura.json").exists()
    assert not (project / ".claude" / "kura.json").exists()
...
def test_c18_an_unknown_dependency_warns_but_still_installs(catalog, effective, home, project):
    parent = cat.Artifact(
        name="parent", type=cat.SKILL, dependencies=("nope",), source=CATALOG / "skills" / PROJECT_SKILL
    )
...
def test_c20_the_closure_is_transitive(catalog, effective, home, project):
    ...
    leaf = cat.Artifact(name="leaf", type=cat.SKILL, source=CATALOG / "skills" / PROJECT_SKILL)
    mid = cat.Artifact(name="mid", type=cat.SKILL, dependencies=("leaf",), source=CATALOG / "skills" / PROJECT_SKILL)
    top = cat.Artifact(name="top", type=cat.SKILL, dependencies=("mid",), source=CATALOG / "skills" / PROJECT_SKILL)
    doctored = {
        **catalog,
        (cat.SKILL, "leaf"): leaf,
        (cat.SKILL, "mid"): mid,
...
def test_dependency_cycles_terminate(catalog, effective, home, project):
    source = CATALOG / "skills" / PROJECT_SKILL
