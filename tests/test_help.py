...
`kura -h` splits the commands by what they touch, because that is what
...
from kura.cli import COMMANDS, FAMILIES, SCOPE, build_parser
...
def test_every_family_is_uniform_on_the_flag():
    ...
    how `kura list --global` became a documented command that does not exist."""
    has = _defines_global()
    for title, names in FAMILIES:
...
def test_an_unknown_flag_is_refused_under_the_subcommands_usage(kit):
    ...
    usage line was `kura [-h] COMMAND ...` and named none of list's own flags."""
    result = kit("list", "--type", "skill", "--global")
    assert result.returncode == errors.USAGE
    assert "usage: kura list" in result.stderr
...
def test_a_refusal_to_a_redirected_stderr_carries_no_escapes(monkeypatch, capsys):
    ...
    terminal on stdout and a file on stderr, `kura bogus 2>log` used to write
