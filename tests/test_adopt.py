"""Group H: `adopt`, reconstructing a lost kura.json.
...
from kura.commands import adopt, listing, remove
...
def test_h2_the_inferred_records_restore_the_cascade():
    ...
    assert without == ([], [("dep", ["not tracked by kura"])]), "the bug being fixed"
...
def test_h3_a_partially_recorded_project_is_topped_up():
    """The fish-era project that later had one skill added through kura."""
...
def test_h5_only_links_into_our_store_are_adopted(catalog, tmp_path, home):
    ...
    link(skills, "coderabbit", CATALOG / "skills" / "coderabbit")
    (skills / "hand-written").mkdir()
    link(skills, "foreign", tmp_path / "somewhere-else")

    assert scope.installed_pairs(project, CATALOG) == {(SKILL, "coderabbit")}
...
def test_h5_a_skill_and_a_plugin_sharing_a_name_are_told_apart(catalog, tmp_path):
    ...
    project = tmp_path / "project"
    skills = project / ".claude" / "skills"
    link(skills, "backend", CATALOG / "plugins" / "backend")
    link(skills, "coderabbit", CATALOG / "skills" / "coderabbit")

    assert scope.installed_pairs(project, CATALOG) == {
        (PLUGIN, "backend"),
        (SKILL, "coderabbit"),
    }
...
def test_h5_agents_are_adopted_from_their_own_leaf(catalog, tmp_path):
    project = tmp_path / "project"
    link(project / ".claude" / "agents", "architect.md", CATALOG / "agents" / "architect.md")
    assert scope.installed_pairs(project, CATALOG) == {(AGENT, "architect")}
...
def test_h5_adoption_silences_the_untracked_install_notes(catalog, tmp_path):
    ...
    for name in ("spec-driven-development", "context-engineering"):
        link(skills, name, CATALOG / "skills" / name)

    before = checks.provenance_drift(catalog, {}, project, CATALOG)
    assert [f for f in before if f.check == "untracked-install"]

    entries = adopt.infer(catalog, scope.installed_pairs(project, CATALOG), {})
    state.record(project, entries)

    after = checks.provenance_drift(catalog, state.read(project), project, CATALOG)
...
def test_h5_adoption_restores_the_parent_annotation_in_list(catalog, tmp_path, home):
    ...
    state.record(project, adopt.infer(catalog, scope.installed_pairs(project, CATALOG), {}))
    rows = listing.rows(
        catalog,
        SKILL,
...
def test_h7_a_dry_run_writes_nothing(kit, project):
    link(project / ".claude" / "skills", "coderabbit", CATALOG / "skills" / "coderabbit")
