"""The one thing pi needs from a project that kura installs into.
...
Nothing here is Claude Code's business, so nothing is recorded in `kura.json`. The
...
def converge(project, dry_run=False):
    ...
    `kura converge`. Idempotent, and deliberately derived rather than tracked: the
...
def report(change, project, dry_run=False, stream=None):
    ...
    `stream` exists because `kura converge --quiet` runs from a SessionStart hook,
...
def report_agents(result, project, dry_run=False, stream=None):
    ...
    if result.blocked_dir:
        ui.warn(
            f"{where} already exists and is not a directory kura will write into, "
            "so pi will not see the plugin agents.",
            stream=stream,
        )
        ui.note("Move it aside, then run: kura converge", stream=stream)
