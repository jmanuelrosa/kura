"""`kura adopt`: rebuild a missing kura.json from what is on disk.

A project only acquires a provenance file by being set up with kura, so two
ordinary situations leave links with no record: cloning a repo that ships
.claude/skills/ but not the manifest, and every project set up before kura existed,
...
Writes kura.json, and converges the `.agents/` links pi reads. No `.claude/`
...
def run(args):
    ...
    if not args.dry_run:
        ...
        # predate kura, which is exactly the population with no .agents/ link
