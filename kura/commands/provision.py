"""`kura sync`: converge ~/.claude on what the registries say belongs there.
...
def _report(plan_, dry_run):
    ...
    for art, link in plan_.blocked:
        ...
        ui.warn(
            f"{ui.path(link)} is a real {kind_of_thing}, not a link kura made. "
            f"Leaving it alone."
        )
    ...
    if plan_.missing:
        ui.note("Fix the registry name, or run: kura update --type skill")
