...
def plan(catalog, effective, kind, name, want_global, home, project, provenance):
    ...
    if scope.belongs_global(art, effective) and not want_global:
        ...
        return Plan.refusal(
            ...
            f"  Run: kura add {name} --type {kind} --global",
        )
    ...
    if not want_global and project is None:
        return Plan.refusal(
            ...
            f"without saying so.\n"
            f"  cd into any other directory, or: kura add {name} --type {kind} --global",
        )
...
def _report_group(kind, tag, want_global, elsewhere, tally):
    ...
    if elsewhere:
        ...
        ui.note(f"Install that half with: kura add --type {kind} --group {tag}{flag}")
    ...
    if grouped:
        if names:
            return fail(
                ...
                f"Run it alone, then: kura add {' '.join(names)} --type {args.type}",
            )
        ...
        if not args.want_global and project is None:
            return fail(
                ...
                f"  cd into any other directory, or: kura add --type {args.type} "
