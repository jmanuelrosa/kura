"""`kura remove`, including the cascade.
...
the one that makes it a correctness matter rather than a preference:
  ...
  kura standing in ~/work/api cannot see ~/work/web. Removing a global
...
def plan(catalog, kind, name, want_global, home, project, provenance, no_cascade, claude):
    ...
    if not want_global and project is None:
        return Removal.refusal(
            ...
            f"  cd into any other directory, or: kura remove {name} --type {kind} --global",
        )
    ...
    if not link.is_symlink():
        ...
        if link.exists():
            return Removal.refusal(
                name,
                errors.USAGE,
                f"{link} is a real directory, not a link kura made. Leaving it alone.",
            )
    ...
    if want_global and removal.unlink:
        ...
        if art.dependencies:
            ui.note(
                ...
                "need them, and kura cannot see them from here."
            )
    ...
    if grouped:
        if names:
            return fail(
                ...
                f"Run it alone, then: kura remove {' '.join(names)} --type {args.type}",
            )
        ...
        if not args.want_global and project is None:
            return fail(
                ...
                f"$HOME is the one directory that cannot be a project: its .claude is "
                f"~/.claude, so this would act on the global scope without saying so.\n"
                f"  cd into any other directory, or: kura remove --type {args.type} "
        ...
        if not names and not absent:
            return fail(
                ...
                f"  Run: kura list --type {args.type} --group",
            )
    elif not names:
