"""`kura restore`: install what a project's kura.json already records.
...
ships `.claude/kura.json` arrives with a complete record of what it needs and,
...
def summarise(linked, restored, attempted, present, remaining, kind, first_failure, emit=print):
    ...
    if remaining:
        ...
        emit(
            ui.render(
                "note",
                "Nothing was forgotten. Run `kura doctor` for the detail, and "
                "`kura remove` to drop one for good.",
            )
        )
...
def run(args):
    ...
    if project is None:
        return fail(
            ...
            "cascades.\n"
            "  cd into the project you want to restore, or run: kura sync",
        )
    ...
    if not path.is_file():
        return fail(
            ...
            f"restore from.\n"
            f"  If the project already has links, write one with: kura adopt",
        )
    try:
        ...
        return fail(
            ...
            f"  Fix it by hand, or delete it and rebuild with: kura adopt",
        )
