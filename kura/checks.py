...
def untrusted_workspace(config, project, claude):
    ...
    something kura installed depends on it, and a project with no plugins would
...
def pi_agents_unreachable(project):
    ...
    extension discovering nothing, while `kura list` still shows every plugin
    ...
    if pi.agents_dir_blocked(project):
        return [
            Finding(
                ...
                f"{path} exists and is not a directory kura will write into, so pi "
                f"reads no agents here. Move it aside, then run: kura converge",
                cat.PLUGIN,
            )
        ]
        ...
        if (kind, name) not in installed:
            findings.append(
                Finding(
                    ...
                    PROBLEM,
                    f"{kind} '{name}'",
                    "recorded in kura.json but not linked. Run: kura restore",
                    kind,
                )
            )
        ...
        if reason is not None and not state.is_direct(reason):
            findings.append(
                Finding(
                    ...
                    f"Remove with: kura remove {name} --type skill",
                    cat.SKILL,
                )
            )
    return findings
