...
Kura manages coding-agent **skills**, **agents** and **plugins** from one shared catalog.
It installs the catalog into Claude Code and maintains Pi-compatible views of its skills and
agents. Everything is symlinked, so editing the source in the catalog takes effect immediately
everywhere it is linked, with no re-install.

- **Executable:** [`kura`](kura), a shim run from a checkout in development
- **Logic:** [`kura/`](kura/), a stdlib-only Python package beside the shim
...
`sys.path[0]` suppressed (`PYTHONSAFEPATH`, `-P`, `-I`).

---

## Installation

Each release ships a self-contained `kura` executable and a `kura.sha256` checksum file.
Download both, verify the asset, then put it on your `PATH`:
...
On the author's machine an Ansible role instead pins the release tag and checksum and fetches the
asset with `ansible.builtin.get_url`. There is deliberately no `--version` flag: the pinned tag
and verified checksum are the identity, and a string baked into the file could only ever agree
with itself.

Kura reads a **catalog** and refuses to run without one: `~/.local/share/kura/catalog`, or
wherever `KURA_CATALOG` points. A catalog is a directory holding `skill-registry.json`,
`agent-registry.json`, `skills/`, `agents/` and `plugins/<name>/.claude-plugin/plugin.json`.
Nothing else is required of it: not a git repository, not a marker file, not a symlink. It is
mutable state rather than a read-only bundle, since `update` rewrites `skill-registry.json` and
swaps skill directories in place. On the author's machine the default path is a symlink to a
dotfiles checkout's `roles/ai/files/claude`, created by the same role that installs the command,
which is one deployment of many.

Development runs the shim out of a checkout instead: `./kura list --type skill`.

---

## The three artifact types

| Type | Stored in the catalog at | Installs to | Notes |
|---|---|---|---|
| `skill` | `skills/<name>/` | `.claude/skills/<name>` | Tracked in `skill-registry.json` |
| `agent` | `agents/<name>.md` | `.claude/agents/<name>.md` | Tracked in `agent-registry.json` |
| `plugin` | `plugins/<name>/` | `.claude/skills/<name>` | No registry row; its `plugin.json` is the metadata |
...
checks cannot run inside a single type, one `kura.json` holds all three for both `adopt` and
...
rather than being refused; `kura doctor` is what finds those.
...
saying so, would load in every repo, and would be pruned by the next `kura sync`, which
owns that directory. Use `add --global` to write there deliberately.
...
`update` and `outdated` sit outside all of this. They rewrite the skill sources in the catalog and
...
The scopes above are what `kura -h` groups its listing by, six families of them:
...
| Registry-wide | `update`, `outdated` | the catalog's skill sources against upstream | not accepted |
...
of which four have never had it, and that is how `kura list --global` became a documented
...
Each command's own `--help` repeats its scope in one sentence, so `kura add --help` says
...
```
kura list --type {skill,agent,plugin} [--group [TAG]] [--json]
```
...
| `--group TAG` | Show only entries carrying that tag. A kura addition; tags are opaque, so any value in the registry works |
...
```
$ kura list --type skill --json | jq -r '.[] | select(.state == "linked") | .name'
coderabbit
context-engineering
```
...
```
kura scout [--type {skill,agent,plugin}] [--focus TAG] [--add]
```
...
→ kura add react-best-practices test-driven-development --type skill
...
accepted here resolves dependencies, records provenance in `kura.json` and prints the plugin
...
```
kura add NAME [NAME ...] --type {skill,agent,plugin} [--global]
kura add --group TAG --type {skill,agent,plugin} [--global]
```
...
```
$ kura add spec-driven-development --type skill
...
```
$ kura add --group planning --type skill
...
✓ Linked 'spec-driven-development' into ./.claude/skills
  ...
  Install that half with: kura add --type skill --group planning --global
...
```
kura remove NAME [NAME ...] --type {skill,agent,plugin} [--global] [--no-cascade]
kura remove --group TAG --type {skill,agent,plugin} [--global] [--no-cascade]
```
...
```
$ kura remove spec-driven-development --type skill
...
```
$ kura remove --group planning --type skill
...
```
kura sync [--type {skill,agent,plugin}] [--dry-run]
```
...
global, unlinks every one that no longer is. This is the command a provisioning run invokes, and
it replaces the derivation, symlink and prune block that used to sit in the author's Ansible role.
...
- **Only links into the catalog's stores.** A link pointing anywhere else is somebody
...
A registered artifact missing from the catalog is reported rather than linked, and the run exits
`2`. The old Ansible used `force: true`, which never validated its target, so a registry typo
became a symlink resolving nowhere while the play reported success.
...
lines saying nothing happened is what teaches you to skip the report. A provisioner matches
`, 0 changes` on that closing line to decide whether its task was `changed` (the author's `ai`
role does), and a test pins the wording so it cannot drift out from under one.
...
These update the **sources in the catalog**, not your installs. Since installs are symlinks, every
project already pointing at a skill sees the new content immediately.

Only skills have upstreams: `agent-registry.json` has no repos and plugins are authored in place.
...
```
kura doctor [--type {skill,agent,plugin}]
```
...
```
kura adopt [--type {skill,agent,plugin}] [--dry-run]
```
...
Rebuilds a missing `kura.json` from the links already in the project. Use it after cloning
a repo that ships a `.claude/` but no manifest, or in a project set up before kura existed, by
...
```
$ kura adopt --dry-run
📋 Would record in ~/work/api/.claude/kura.json:
...
```
kura restore [--type {skill,agent,plugin}] [--dry-run]
```
...
the catalog and are never committed, so a repo shipping `.claude/kura.json`
...
```
$ kura restore
📦 Restoring from ~/work/api/.claude/kura.json:
...
`$HOME`, where `kura sync` is the command that applies.
...
Exits `9` when a read finds the workspace untrusted, so `kura trust >/dev/null` works as a
condition. Exits `5` when the flag already reads the way you asked for.

`kura doctor` runs the same check the other way round, reporting a problem when a project has
plugins linked but its workspace is not trusted. It only asks once a plugin is actually installed:
trust is Claude Code's business until something kura put there depends on it.
...
into the catalog, so committing one hands a teammate a dangling link. The file goes
...
and does not fail the sweep: that is a fact about the catalog's registries rather than about the
project being swept.
...
| 2 | `NOT_FOUND` | no artifact of that type by that name, registered but absent on disk, or no `kura.json` for `restore` to read |
...
| `KURA_CATALOG` | Which catalog to read artifacts from. Defaults to `~/.local/share/kura/catalog`, and when neither that nor the override is a directory every command refuses rather than reading empty registries |
...
Every line these commands print comes from `kura/ui.py`, the shared vocabulary: `title`
...
The `_ui` helper in the author's fish configuration is the same vocabulary for fish, kept
byte-identical by a differential test that lives beside it. **A topic emoji only ever appears on a
`title` or a `done`**: emoji are double-width, so one used as a row marker would break the suffix
columns in `list`.
...
→ kura add react-best-practices --type skill
✨ 1 strong, 1 worth considering, 0 already here

$ kura scout --type skill --add
...
...
```console
$ cd ~/work/api
$ kura add coderabbit --type skill
...
```console
$ kura add commit --type skill
✗ 'commit' carries the global tag, so it belongs in ~/.claude.
  Run: kura add commit --type skill --global

$ kura add commit --type skill --global
✓ Linked 'commit' into ~/.claude/skills
```

**Provision every global artifact at once, which is what a provisioning run does.**
...
```console
$ kura add coderabbit no-such-thing frontend-design --type skill
...
```console
$ kura list --type skill --group          # which tags exist
📚 Available groups:
  frontend:
    ...
$ kura add --group frontend --type skill
✓ Linked 'apollo-client' into ./.claude/skills
...
✨ Linked 18 of 18 skills tagged 'frontend'
$ kura add --group frontend --type skill  # idempotent
  Already installed: 18 skills
✨ Linked 0 of 18 skills tagged 'frontend'
$ echo $status
0
$ kura remove --group frontend --type skill
...
...
```console
$ kura add spec-driven-development --type skill    # pulls in test-driven-development
$ kura add test-driven-development --type skill
...
```console
$ kura remove spec-driven-development --type skill --no-cascade
$ kura doctor --type skill
📝 Notes:
  skill 'context-engineering': installed for spec-driven-development, which nothing installed
    needs now. Remove with: kura remove context-engineering --type skill  [removable]
...
📝 Notes:
  skill 'context-engineering': linked but not recorded, so the cascade will keep it rather than
    guess. Run: kura adopt  [untracked-install]

✨ 0 problem(s), 2 note(s) across skills

$ kura adopt
📋 Recorded in ~/api/.claude/kura.json:
  skill 'context-engineering'      dep-of:spec-driven-development
  skill 'spec-driven-development'  direct
...
$ kura update --type skill frontend-design
```

**Use in a script.**

```fish
if kura doctor >/dev/null
    echo "artifacts are healthy"
else
    kura doctor   # exits 9, so show the detail
end
```
...
**Why does `kura list --global` fail?**
...
`kura list --type skill --group global`. The same goes for `scout`, `doctor`, `adopt` and
...
```
$ kura list --global
usage: kura list [-h] --type {skill,agent,plugin} [--group [TAG]]
...
Because it cannot know whether another project still needs it. kura standing in
...
**Why is there a `kura.json` in my project's `.claude/`?**
...
it recognises next time it runs. `kura adopt` rebuilds it in one pass.

**I cloned a repo with a `.claude/` and no manifest. What do I lose?**
The cascade, until you run `kura adopt`. Every link reads as untracked, so `remove` keeps
...
`kura restore`. The symlinks point into the catalog and are never committed, so the
...
Because they point somewhere else. `add` writes an absolute target into the catalog, so a symlink
committed by a teammate whose catalog lives at a different path is dangling here, and `adopt` only
records links that resolve into this machine's catalog. `doctor` reports those as
...
directories. Run `kura add <name> --type skill` and it is promoted back to `direct`, which
...
artifact into the global directory, load it in every repo, and then have it pruned by the next
`kura sync`, which owns `~/.claude/skills`. It would also leave a `kura.json` in
...
nested under another key. The tests run every case and every block in the fixture catalog past
PyYAML to prove nothing it reports is a false alarm.

**Why does `update --type agent` fail?**
Only skills have upstreams. `agent-registry.json` has no repos and plugins are authored in the
catalog rather than fetched, so there is nothing to sync. Exit `1`.
...
Yes, immediately, and that is intentional. Installs are symlinks into the catalog, so syncing a
...
is the property that lets a provisioning run be re-runnable. A link is kept only while its
...
Because it can tell the difference. The cascade needs `kura.json` to know which links arrived
...
Deleted. kura covers everything they did, plus plugins, provenance and the cascade, and it is
the half that is under test. The Television pickers now drive it, and `kura adopt` recovers
